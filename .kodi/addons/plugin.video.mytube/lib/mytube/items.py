...
from mytube.search import queryType, querySort
...
class Query(Item):
    ...
    def getItem(self, url):
        return ListItem(
            ...
            isFolder=True,
            infoLabels=self.labels(self.query, self.query),
            contextMenus=self.menus(
                q=self.query,
                _type_=localizedString(queryType[self.type]["label"]),
                _sort_=localizedString(querySort[self.sort]["label"])
            ),
...
class Results(Contents):
    ...
    def __init__(self, contents, **kwargs):
        super(Results, self).__init__(
            [self.__types__[content["type"]](content) for content in contents],
            **kwargs
        )
